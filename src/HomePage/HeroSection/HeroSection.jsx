...
const HeroSection = () => {
    ...
    // ── go to slide ──
    const goTo = useCallback((target, dir = 'left') => {
    ...
    const goNext = useCallback(() => goTo((currentIndex + 1) % HERO_IMAGES.length, 'left'),  [currentIndex, goTo]);
    const goPrev = useCallback(() => goTo((currentIndex - 1 + HERO_IMAGES.length) % HERO_IMAGES.length, 'right'), [currentIndex, goTo]);
    ...
    useEffect(() => {
        const id = setInterval(() => {
            ...
            setTimeout(() => { setWordIndex(i => (i + 1) % ROTATING_WORDS.length); setWordVisible(true); }, 380);
        }, 2500);
    ...
    const onPointerDown = (e) => {
        ...
        drag.current = { startX: e.clientX ?? e.touches[0].clientX, startY: e.clientY ?? e.touches[0].clientY, delta: 0 };
        setIsGrabbing(true);
        clearInterval(sliderTimer.current);
    };
    ...
    return (
        <section className="hero-v2">
            ...
            >
                ...
                >
                    <img src={HERO_IMAGES[currentIndex]} alt="Focus project" draggable={false} />
                ...
                {isTransitioning && (
                    <div className={`hero-v2__slide hero-v2__slide--next ${inClass}`}>
                        <img src={HERO_IMAGES[nextIndex]} alt="Focus project" draggable={false} />
                        <div className="hero-v2__slide-overlay" />
                    </div>
                )}
                ...
                {!isTransitioning && isGrabbing && dragOffset !== 0 && (
                    <div
                        ...
                        style={{ transform: `translateX(${peekTranslate})`, transition: 'none' }}
                    >
                        <img src={HERO_IMAGES[peekIndex]} alt="Focus project" draggable={false} />
                        <div className="hero-v2__slide-overlay" />
                    </div>
                )}
                ...
                <button
                    ...
                    onClick={(e) => { e.stopPropagation(); goPrev(); resetAutoplay(); }}
                ...
                <button
                    ...
                    onClick={(e) => { e.stopPropagation(); goNext(); resetAutoplay(); }}
                ...
                <div className="hero-v2__dots">
                    {HERO_IMAGES.map((_, i) => (
                        <button
                            ...
                            onClick={(e) => { e.stopPropagation(); goTo(i, i > currentIndex ? 'left' : 'right'); resetAutoplay(); }}
                ...
                <div className="hero-v2__counter">
                    <span className="hero-v2__counter-current">{String(currentIndex + 1).padStart(2, '0')}</span>
                    <span className="hero-v2__counter-sep"> / </span>
                    <span className="hero-v2__counter-total">{String(HERO_IMAGES.length).padStart(2, '0')}</span>
                </div>
                ...
                <div className="hero-v2__progress-track">
                    <div
                        ...
                        style={{ width: `${((currentIndex + 1) / HERO_IMAGES.length) * 100}%`, transition: 'width 0.7s ease' }}
                    />
                </div>
            </div>
            ...
            <div className="hero-v2__content">
                ...
                <h1 className="hero-v2__title">
                    ...
                    <span className="hero-v2__title-line accent">
                        <span className={`hero-v2__rotating-word${wordVisible ? ' visible' : ''}`} key={wordIndex}>
                ...
                <div className="hero-v2__actions">
                    ...
                    <a href="/projects" className="hero-v2__btn hero-v2__btn--ghost">View Projects</a>
                </div>
...
const StatItem = ({ value, suffix, label, counted, decimal }) => {
    ...
    useEffect(() => {
        ...
        const tick = (now) => {
            ...
            setDisplay(decimal ? Math.round(e * value * 10) / 10 : Math.floor(e * value));
    ...
    return (
        <div className="hero-v2__stat">
            <span className="hero-v2__stat-value">{decimal ? display.toFixed(1) : display}{suffix}</span>
